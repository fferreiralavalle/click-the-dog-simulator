...
const mails = (state=INITIAL_STATE, action:any)=>{
    switch(action.type){
        case types.UPDATE_MAILS:
            return {
                ...state,
                mails: GameManager.getInstance().getNotificationManager().getAllNotifications(),
                unseen: GameManager.getInstance().getNotificationManager().getUnseenNotificationsAmount()
            }
        default:
            return state          
    }
