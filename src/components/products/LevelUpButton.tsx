...
class LevelUpButton extends Component<IRecipeProps> {
  ...
  render() {
    ...
    const levelUpCurrency: string = toFormat(
      product.getLevelUpPrice().currency
    );
    const levelUpTreats: string = toFormat(product.getLevelUpPrice().treats);
    const progressTreats = !levelUpPrice.treats.equals(0)
      ? new Decimal(Number(treats)).div(levelUpPrice.treats)
      : new Decimal(1);
    const progressCurrency = !levelUpPrice.currency.equals(0)
      ? new Decimal(Number(currency)).div(levelUpPrice.currency)
      : new Decimal(1);
    const progressStyle = {
    ...
    return (
      <div
        className={`product-upgrade ${!canLevelUp && "disabled"}`}
