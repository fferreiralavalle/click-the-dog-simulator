...
class TimeWizard extends Component<IRecipeProps, IState> {
  ...
  render() {
    ...
    return (
      <div
        className="product product3 boxed"
  ...
  renderTurboButton(turboTimePrice: number) {
    ...
    const price = GameManager.getInstance()
      .getTimeManager()
      .getPatiencePoints(turboTimePrice);
    const priceFormat = toFormat(price);
    const { patiencePoints } = this.props;
    const canBuyClass = patiencePoints.gte(price) ? "" : "disabled";
    return (
      <div
        className={"highlight-store-button " + canBuyClass}
        onClick={() => this.onTurboBuy(price)}
      >
