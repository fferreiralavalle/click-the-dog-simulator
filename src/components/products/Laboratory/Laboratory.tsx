...
import { Laboratory } from "../../../game/products/Laboratory";
...
class LaboratoryUI extends Component<IRecipeProps, IState> {
  ...
  onEventReward = (result: Currency, product: Laboratory) => {
    ...
    if (
      result.treats.lessThanOrEqualTo(0) &&
      result.currency.lessThanOrEqualTo(0)
    )
      return;
    ...
    switch (eventId) {
      ...
      case variableIds.labUpgradeTier2A:
        return {
          name: "Magic Rubbing",
          description:
            "Petting has a chance of producing 1 Treat. Scientific Magic!",
      ...
      case variableIds.labUpgradeTier2B:
        return {
          name: "Special Event!",
          description:
            "Every Farm event has a chance of doubling its rewards. Yay!",
      ...
      case variableIds.labUpgradeTier2C:
        return {
          ...
          description:
            "Labs produce love for each Farm and Divine Petting level!",
  ...
  renderContent(product: Laboratory) {
    ...
    return (
      <React.Fragment>
        <div className="laboratory-building"></div>
        <div className="product-level">{level.getValue()}</div>
        <div className="product1-title">LABoratory</div>
        {freePoints > 0 && (
  ...
  renderHighlight(product: Laboratory) {
    ...
    const currentPointsClass =
      product.getAvailablePoints(usedLevel) > 0
        ? "lab-text-positive"
        : "lab-text-negative";
    return (
      <div className={"highlight "}>
        <div className="highlight-section">
          <div className="highlight-field title">Lab Info</div>
          <div className="highlight-field">
            The LABoratory is where our brightest puppers use their smarts for
            science.
          </div>
          <div className="highlight-field title">Lab Stats</div>
          <div className={"highlight-field" + levelClass}>
            <div className="highlight-attribute">Points</div>
            <div className="highlight-value">
              <span className={currentPointsClass}>{currentPoints}</span>/
              {points}
            </div>
          </div>
          <div className="highlight-field">
            <div className="highlight-attribute">Progress</div>
            <div className="highlight-value">{progressPS}/s</div>
          </div>
        </div>
        <div className="highlight-section">
          <div className="highlight-field title">Pupgrades</div>
  ...
  renderUpgradeButton(add: number, chosenUpgrade: string, product: Laboratory) {
    ...
    return (
      <div
        className={"lab-button" + className}
  ...
  renderUpgradeStatistic(product: Laboratory) {
    ...
    const upgradeLevel =
      product.getUpgradeLevel(chosenUpgrade) + buyUpgradeHoverValue;
    const bonus = product.getUpgradeBonus(chosenUpgrade, upgradeLevel);
    const { baseBonus } = bonus;
    const title = <div className="highlight-field title">Stats</div>;
    const points = (
      <div className="highlight-field">
        <div className="highlight-attribute">Points</div>
        <div className="highlight-value">
          -{product.getUpgradePrice(chosenUpgrade, buyUpgradeHoverValue)}
        </div>
      </div>
    );
        ...
        return (
          <React.Fragment>
            ...
            <div className="highlight-field">
              ...
              <div className="highlight-value">{toFormat(baseBonus)}/s</div>
            </div>
          </React.Fragment>
        );
      default:
          ...
          {ut.upgrades.map((upgrade) => {
            ...
            return (
              <div
                ...
                onClick={this.selectUpgrade(upgrade.id)}
              >
