...
import {
  PetAppreciationCenter,
  RewardResult,
  events,
} from "../../../game/products/PetAppreciationCenter";
...
class Product1 extends Component<IRecipeProps, IState> {
  ...
  constructor(props: any) {
    ...
    const product = GameManager.getInstance()
      .getProductManager()
      .getProduct(ids.product1Level) as PetAppreciationCenter;
    ...
    switch (eventId) {
      case events.pettingTraining.id:
        return {
          name: "Petting Party!",
          description:
            "Makes petting more efficient for some time (multiple instances at a time add up)",
        };
      case events.donationCampaign.id:
        return {
          name: "Treats Donation",
          description:
            "Create a campagin for people to donate treats. Some dogs really like them! +1 Treats every 5 levels.",
        };
      default:
        return {
          name: "Belly Rub Day",
          description:
            "People come to belly rub your good boys, produces tons of love!",
        };
    }
  };
  ...
  render() {
    ...
    return (
      <div
        className="product product1 boxed"
        onMouseEnter={this.onHover(true)}
        onMouseLeave={this.onHover(false)}
      >
        {isHover ? this.renderHighlight() : this.renderContent()}
        <LevelUpButton
          productId={ids.product1Level}
  ...
  renderContent() {
    ...
    return (
      <React.Fragment>
        <div className="product1-building"></div>
        <div className="product-level">{level.getValue()}</div>
        <div className="product1-title">Pet Farm</div>
        <div className="progress">
          {this.renderCurrentEvent()}
          <div className="progress-bar">
            <div className="progress-bar-progress" style={progressStyle}></div>
            <div className="progress-bar-value">
              {`${Math.floor(progress.getValue())} / ${Math.floor(
                product.getProgressGoal()
              )}`}
            </div>
          </div>
        </div>
    ...
    return (
      <div className={"highlight " + levelClass}>
        <div className="highlight-section">
          <div className="highlight-field title">Farm Info</div>
          <div className="highlight-field">
            Shelters lost pets in a cozy home. It also prepares EVENTS for them!
          </div>
          <div className="highlight-field title">Farm Stats</div>
          <div className="highlight-field">
        ...
        <div className="highlight-section">
          <div className="highlight-field title">Events</div>
          {this.renderEventOptions(product, eventId.getValue())}
        </div>
        <div className="highlight-section">
          <div className="highlight-field title">Event Info</div>
          <div className="highlight-field">{description}</div>
        ...
        return (
          <React.Fragment>
            ...
            <div className="highlight-field">
              ...
              <div className="highlight-value">
                {product
                  .getEventReward(eventId, level)
                  .currencyReward.treats.toString()}
  ...
  renderEventOptions(product: PetAppreciationCenter, selected: string) {
    ...
    return events.map((e) => {
      ...
      return (
        <div
          className={"highlight-field highlight-event-select" + selectedClass}
