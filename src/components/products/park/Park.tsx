...
class ParkUI extends Component<IRecipeProps, IState> {
  ...
  onEventReward = (result: RewardResult) => {
    ...
    if (ppd) ppd.addCurrency(plusCurrency);
  };
  ...
  onCurrencyGain = (currency: Currency, product: Park) => {
    ...
    if (timer0) {
      const { text, progress } = this.getUpdatedBarValues(
        events.pupsloration.id,
        product
      );
  ...
  getUpdatedBarValues(
    ...
    const text = isReady
      ? this.productText.claimPreview
      : `${toFormatTime(Math.max(progressNeeded - progress, 0))}`;
  ...
  renderHighlight(park: Park) {
    ...
    const lps = toFormat(
      park.updateCurrencyPerSecond(usedLevel, true).currency.toString()
    );
    const lovePerRelic = toFormat(
      park.getCurrencyPerRelic(usedLevel).currency.toString()
    );
      ...
      <div className={"highlight " + levelClass}>
        <div className="highlight-section">
          ...
          <div className="highlight-field">
            ...
            <div className="highlight-value">{relicAmount}</div>
          </div>
        </div>
        <div className="highlight-section">
          <div className="highlight-field title">{this.productText.quest}</div>
          {this.renderEventOptions(park, usedLevel)}
        </div>
        <div className="highlight-section">
          <div className="highlight-field title">{this.commonText.info}</div>
          <div className="highlight-field">
  ...
  renderEventStatistic(product: Park, level: number) {
    ...
    return (
      <React.Fragment>
        {title}
        <div className="highlight-field">
          <div className="highlight-attribute">{this.relicsText.relic}</div>
          <div className="highlight-value">{toFormat(relicChance * 100)}%</div>
        </div>
        <div className="highlight-field">
          <div className="highlight-attribute">{this.commonText.price}</div>
          <div className="highlight-value">
            {toFormat(price.toString())}
            <div className="treat-icon" />
          </div>
        </div>
        <div
          className={"highlight-field" + readyClass}
          onClick={this.claimReward(displayedEvent, product)}
        >
          <div className="highlight-button-claim">{buttontext}</div>
        </div>
      </React.Fragment>
    );
  }
  ...
  renderEventOptions(product: Park, level: number) {
    ...
    Object.keys(events).forEach((e) => {
      if (events[e].unlockLevel <= level) {
        ...
        uiEvents.push(
          <div
            className={"highlight-field highlight-event-select" + isSelected}
            onClick={this.selectEvent(e)}
          >
    ...
    uiEvents.push(
      <div
        ...
        onClick={this.toggleRelicView(true)}
      >
        ...
        <div className="highlight-section">
          ...
          <div className="highlight-field">
            <span className="relic-description">
              {isCurrentRelicUnlocked ? description : "???"}
            </span>
          </div>
        </div>
        <div
          className="highlight-closeButton"
          onClick={this.toggleRelicView(false)}
        >
          X
        </div>
      </div>
  ...
  renderRelicTier(tier: RelicTier, park: Park) {
    ...
    return (
      <div className="highlight-tier">
        {relics.map((relic) =>
          this.renderRelic(relic, park.isRelicUnlocked(relic.id))
        )}
      </div>
    );
  }
  ...
  renderRelic(relic: Relic, isUnlocked: boolean) {
    ...
    return (
      <div
        className={"highlight-field" + lockClass}
        onClick={this.selectRelic(relic.id)}
      >
