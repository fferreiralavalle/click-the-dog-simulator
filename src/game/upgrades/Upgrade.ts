...
export class UpgradeKingStandar implements UpgradeKing {
  ...
  isUnlocked(upgradeLevel: number, kingLevel: number): boolean {
    const relatedIdLevel =
      this.minLevel +
      GameManager.getInstance().getVariable(this.relatedId).getValue();
  ...
  canUpgrade(
    ...
    const codition =
      modPrice.currency.lte(currency.currency) &&
      modPrice.treats.lte(currency.treats);
