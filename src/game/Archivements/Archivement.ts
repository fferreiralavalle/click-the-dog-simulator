...
export class ArchivementBase implements ArchivementInterface {
  ...
  inUnlocked(): boolean {
    const archivement = Number(
      GameManager.getInstance().getVariable(this.varId).getValue()
    );
  ...
  getBaseNotification(): Notification {
    ...
    return {
      id: "Achievement-" + this.varId,
      title:
        archivements.archivementBaseTitle +
        ": " +
        getArchivementText(this.varId).title,
