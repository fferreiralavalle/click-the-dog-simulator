...
export class PetPetting implements Product {
  ...
  updateCurrencyPet(level?: number, dontApply: boolean = false): Currency {
    ...
    const base: number = 1;
    //Pet Training Bonus
    const pac = GameManager.getInstance()
      .getProductManager()
    ...
    //Final Gain
    const final = new Decimal(base)
      .add(Math.floor(lvl / 3))
      .mul(petTrainingMult)
      .mul(labMult)
      .mul(relic0D)
      .mul(relic1A)
  ...
  onTimePassed(timePassed: number): Currency {
    let cps: Currency = this.getCurrencyPerSecond();
    //Pet Lab Chance Bonus
    const lab = GameManager.getInstance()
      .getProductManager()
      .getProduct(ids.product2Level) as Laboratory;
    ...
    if (isCritical) {
      const base: number = 0.1;
      const hands = this.getHandsAmount();
      cps = { ...cps, treats: new Decimal(base * hands) };
    }
    ...
    if (this.canLevelUp()) {
      ...
      if (this.getLevel() === 1) {
        GameManager.getInstance().getNotificationManager().addNotification({
          ...
          description:
            "Wow! Your dog is being automaticly pet! Hover over a building to see more details.",
