...
export class PetAppreciationCenter implements Product {
  ...
  updateCurrencyPerSecond(
    ...
    //Final
    const currencyPerSecond = new Decimal(base)
      .mul(currentLevel)
      .mul(relic0EBonus)
      .mul(relic1EBonus)
      .mul(relic2EBonus)
  ...
  checkForProgressCompletion(): Currency {
    ...
    let progress = GameManager.getInstance()
      .getVariable(ids.product1Progress)
      .getValue();
    while (progress >= goal) {
      const event = this.getEvent();
      const lab = GameManager.getInstance()
        .getProductManager()
        .getProduct(ids.product2Level) as Laboratory;
      const criticalChange = lab.getUpgradeBonus(ids.labUpgradeTier2B)
        .baseBonus;
      ...
      switch (event.id) {
        case events.pettingTraining.id:
          ...
          let pts = GameManager.getInstance()
            .getVariable(ids.product1EventPettingTrainingDurations)
            .getValue() as Array<PetTrainingVariable>;
          pts = pts ? pts : [];
          const petReward = {
          ...
          GameManager.getInstance().setVariable(
            pts,
  ...
  checkForEventDurations(timePassed: number) {
    //Training
    let ptDurations = GameManager.getInstance()
      .getVariable(ids.product1EventPettingTrainingDurations)
      ?.getValue() as Array<PetTrainingVariable>;
      ...
      case events.donationCampaign.id:
        ...
        return {
          currencyReward: {
            currency: new Decimal(0),
            treats: new Decimal(
              this.getEvent(eventId).baseReward + Math.floor(finalLevel / 5 - 2)
            ).mul(relicTier2B),
          },
        };
      default:
        ...
        const relic0BBonus = isUnlocked
          ? park.getRelicBonus(ids.relicTier0B)
          : 0;
        const relicBonusCurrency = this.getCurrencyPerSecond().currency.mul(
          relic0BBonus
        );
        return {
          currencyReward: {
            currency: new Decimal(this.getEvent(eventId).baseReward)
              .add(relicBonusCurrency)
              .mul(1 + 0.5 * finalLevel),
            treats: new Decimal(0),
          },
        };
    }
  ...
  getLevelUpPrice(): Currency {
    ...
    const currentLevel: number = GameManager.getInstance()
      .getVariable(this.variableId)
      .getValue();
    const tree = GameManager.getInstance()
      .getProductManager()
      .getProduct(ids.treeOfGoodBoys) as Tree;
    const discount: number = tree.getBlessing(ids.blessing0A).getBonus();
    const finalPrice = new Decimal(initialPrice)
      .add(basePrice * currentLevel)
      .add(Math.pow(basePrice, currentLevel / 4 + 1));
    return {
  ...
  canLevelUp(): boolean {
    const currency: Decimal = GameManager.getInstance()
    ...
    if (this.canLevelUp()) {
      ...
      if (this.getLevel() === 1) {
        GameManager.getInstance().getNotificationManager().addNotification({
          ...
          description:
            "Hello good sir, my farm can prepare events that give BIG bonuses. You can choose a new event at level 10.",
  ...
  getEvent(eventId?: string): EventType {
    let finalEventId = eventId
      ? eventId
      : GameManager.getInstance().getVariable(ids.product1Event).getValue();
    ...
    if (!event) {
      ...
      event = GameManager.getInstance()
        .getVariable(ids.product1Event)
        .getValue();
    }
    return event;
  }
  ...
  getPettingTrainingData(currentLevel?: number): PettingTrainingData {
    const level = currentLevel ? currentLevel : this.getLevel();
    const petMultiplier = events.pettingTraining.baseReward + 0.02 * level;
    const duration = 4;
    return {
      petMultiplier,
      duration,
