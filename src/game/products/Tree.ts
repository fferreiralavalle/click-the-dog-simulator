...
export class Tree implements Product {
        ...
        if (this.canLevelUp()){
            ...
            if (this.getLevel()===1){
                GameManager.getInstance().getNotificationManager().addNotification({
                    ...
                    description:'Come forth human, the time has come to make a hard desition...',
    ...
    getAllBuildingLevels(){
        const petLevel = GameManager.getInstance().getVariable(ids.product0Level)?.getValue();
        const farmLevel = GameManager.getInstance().getVariable(ids.product1Level)?.getValue();
        const labLevel = GameManager.getInstance().getVariable(ids.product2Level)?.getValue();
        const wizpugLevel = GameManager.getInstance().getVariable(ids.product3Level)?.getValue();
        const kingLevel = GameManager.getInstance().getVariable(ids.upgradeShop)?.getValue();
        const parkLevel = GameManager.getInstance().getVariable(ids.product4Level)?.getValue();
        const sum = petLevel + farmLevel + labLevel + wizpugLevel + kingLevel + parkLevel;
        return sum;
    }
