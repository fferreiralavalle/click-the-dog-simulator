...
export class Museum implements Product {
  ...
  updateCurrencyPerSecond(
    ...
    const archivements = GameManager.getInstance()
      .getArchivementManager()
      .getUnlockedArchivements().length;
    const currencyperArchivement = this.getCurrencyPerArchivement(lvl);
    const timeMult = this.getTimePassedMult();
    const currencyPerSecond = currencyperArchivement
      .mul(archivements)
      .mul(timeMult);
  ...
  getCurrencyPerArchivement(level?: number): Decimal {
    const lvl = level ? level : this.getLevel();
    const king = GameManager.getInstance()
      .getProductManager()
      .getProduct(ids.upgradeShop) as King;
  ...
  canUnlock(): boolean {
    const museumRelic = Number(
      GameManager.getInstance().getVariable(ids.relicMuseum).getValue()
    );
  ...
  getLevelUpPrice(): Currency {
    ...
    const finalPrice = new Decimal(basePrice)
      .mul(currentLevel)
      .plus(basePrice.pow(currentLevel / 4 + 1))
      .mul(discount);
    return {
      currency: new Decimal(0),
  ...
  canLevelUp(): boolean {
    const treats: Decimal = GameManager.getInstance()
      .getVariable(ids.treats)
      .getValue();
      ...
      if (this.getLevel() === 1) {
        GameManager.getInstance()
          ...
          .addNotification({
            ...
            title: "Hsss!",
            description:
              "There is enough human for both dogs and cats! Not that I need their affection or anything...",
        ...
        GameManager.getInstance()
          ...
          .addNotification({
            ...
            title: "A cat???",
            description:
              "Do not worry human, I will take this trespasser down with my DRAGON BREATH! *bark bork*",
        ...
        GameManager.getInstance()
          ...
          .addNotification({
            ...
            description:
              "Hsss! Wait, a human? And it has shiny achievements. Allow me to take care of those...",
            image: getBuildingIcon(ids.museum).icon,
            background: getBuildingIcon(ids.museum).background,
            seen: false,
          });
