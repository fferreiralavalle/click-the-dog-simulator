...
const baseUpgradeCostLove: Currency = {
  currency: new Decimal(1000),
...
export class King implements Product {
  ...
  updateCurrencyPerSecond(
    ...
    const pettingLvl = Number(
      GameManager.getInstance().getVariable(ids.product0Level).getValue()
    );
    const farmLvl = Number(
      GameManager.getInstance().getVariable(ids.product1Level).getValue()
    );
    const LabLvl = Number(
      GameManager.getInstance().getVariable(ids.product2Level).getValue()
    );
    const wizLvl = Number(
      GameManager.getInstance().getVariable(ids.product3Level).getValue()
    );
    const parkLvl = Number(
      GameManager.getInstance().getVariable(ids.product4Level).getValue()
    );
    const museumLvl = Number(
      GameManager.getInstance().getVariable(ids.museum).getValue()
    );
    const treeLvl = Number(
      GameManager.getInstance().getVariable(ids.treeOfGoodBoys).getValue()
    );
    const allLevels =
      currentLevel +
      pettingLvl +
    ...
    const lovePerSecond = new Decimal(allLevels)
      .mul(multiplier)
      .mul(blessing0C);
  ...
  canUnlock(): boolean {
    ...
    const farmLevel: number = GameManager.getInstance()
      ...
      .getValue();
    const labLevel: number = GameManager.getInstance()
    ...
    return (
      farmNeeded <= farmLevel &&
      handsNeeded <= handsLevel &&
      labLevelNeeded <= labLevel
    );
  }
  getLevelUpPrice(level?: number): Currency {
    ...
    const finalPrice: Decimal = initialPrice
      .add(basePrice.mul(lvl * 100))
      .add(basePrice.pow(1 + lvl / 4))
      .mul(discount);
    return {
      currency: finalPrice,
      treats: new Decimal(0),
  ...
  canLevelUp(): boolean {
    const currency = GameManager.getInstance()
      .getVariable(ids.currency)
      .getValue();
      ...
      if (this.getLevel() === 1) {
        GameManager.getInstance()
          ...
          .addNotification({
            ...
            description:
              "Good evening dear subjects, I, King Baby have finally returned from my morning stroll and my doogness, am I ready to rule again. I gain love for each Box level you have (Divine Petting, Pet Farm, Lab, etc)",
  ...
  getUpgradePrice(upgradeId: string): Currency {
    ...
    const lvl = Number(
      GameManager.getInstance().getVariable(upgradeId).getValue()
    );
    const park = GameManager.getInstance()
      .getProductManager()
      .getProduct(ids.product4Level) as Park;
    ...
    const cost = multiplyCurrencyBy(
      upgrade.getCost(lvl),
      discountRelic * blessing2A
    );
    return cost;
  }
  ...
  canBuyUpgrade(upgradeId: string, level?: number): boolean {
    ...
    const tree = GameManager.getInstance()
      ...
      .getProduct(ids.treeOfGoodBoys) as Tree;
    const blessing2A: number = tree.getBlessing(ids.blessing2A).getBonus();
    const currency: Currency = multiplyCurrencyBy(
      GameManager.getInstance().getCurrency(),
      1 / discountRelic / blessing2A
    );
  ...
  getAvailableUpgrades(level?: number): Array<UpgradeKing> {
    ...
    const availableUpgrades = upgrades.filter((upgrade) => {
      const upgradeLvl = GameManager.getInstance()
        .getVariable(upgrade.id)
        .getValue();
  ...
  getUpgrades(): Array<UpgradeKing> {
    return [
      new UpgradeKingStandar(
        ids.upgradeProduct0A,
        baseUpgradeCostLove,
        ids.product0Level
      ),
      new UpgradeKingStandar(
        ids.upgradeProduct1A,
        baseUpgradeCostLove,
        ids.product1Level
      ),
      new UpgradeKingStandar(
        ids.upgradeProduct2A,
        baseUpgradeCostLove,
        ids.product2Level
      ),
      new UpgradeKingStandar(
        ids.upgradeProduct2B,
        baseUpgradeCostLove,
        ids.product2Level
      ),
      new UpgradeKingStandar(
        ids.upgradeProduct4A,
        multiplyCurrencyBy(baseUpgradeCostLove, 5),
        ids.product4Level
      ),
      new UpgradeKingStandar(
