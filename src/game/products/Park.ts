...
export class Park implements Product {
  ...
  passAfkTimeAdventure(timePassed: number) {
    if (this.isEventUnlocked(events.pupsloration)) {
      GameManager.getInstance().addToVariable(
        timePassed,
        ids.product4Tier0Progress
      );
    }
    if (this.isEventUnlocked(events.dogsploration)) {
      GameManager.getInstance().addToVariable(
        timePassed,
        ids.product4Tier1Progress
      );
    }
    if (this.isEventUnlocked(events.bigBoysploration)) {
      GameManager.getInstance().addToVariable(
        timePassed,
        ids.product4Tier2Progress
      );
    }
  }
  ...
  getEventReward(eventId: string, level?: number): RewardResult {
    ...
    const event = this.getEvent(eventId);
    const lockedRelics = this.getAllLockedRelics();
    ...
    switch (event.id) {
      ...
      case events.bigBoysploration.id:
        ...
        relicFailedAttemps = Number(
          GameManager.getInstance().getVariable(ids.relicTier2Misses).getValue()
        );
        relicModChance = Math.pow(
          relicChance,
          1 / (1 + relicFailedAttemps * 2)
        );
      ...
      case events.dogsploration.id:
        ...
        relicFailedAttemps = Number(
          GameManager.getInstance().getVariable(ids.relicTier1Misses).getValue()
        );
        relicModChance = Math.pow(
          relicChance,
          1 / (1 + relicFailedAttemps * 2)
      ...
      default:
        ...
        relicFailedAttemps = Number(
          GameManager.getInstance().getVariable(ids.relicTier0Misses).getValue()
        );
        relicModChance = Math.pow(
          relicChance,
          1 / (1 + relicFailedAttemps * 2)
  ...
  sendRelicNotification(relic: Relic) {
    GameManager.getInstance()
      ...
      .addNotification({
        ...
        description:
          "Your pets found a new Relic. Hooray! Its effect says: " +
          getRelicText(relic.id).description,
  ...
  subscribeToReward(rs: RewardSubscribers): void {
    const alreadySubscribed = !this.questSubscribers.filter(
      (sub) => sub.id === rs.id
    );
    if (!alreadySubscribed) {
  ...
  canUnlock(): boolean {
    const previousProductLevel: number = GameManager.getInstance()
  ...
  getLevelUpPrice(): Currency {
    ...
    const currentLevel: Decimal = new Decimal(
      Number(GameManager.getInstance().getVariable(this.variableId).getValue())
    );
    const tree = GameManager.getInstance()
      .getProductManager()
      .getProduct(ids.treeOfGoodBoys) as Tree;
    const discount: number = tree.getBlessing(ids.blessing0A).getBonus();
    const finalPrice: Decimal = initialPrice
      .add(basePrice.mul(currentLevel).mul(150))
      .add(basePrice.pow(currentLevel.div(4).add(1)))
      .mul(discount);
    return {
      currency: finalPrice,
      treats: new Decimal(0),
  ...
  canLevelUp(): boolean {
    const currency = new Decimal(
      Number(GameManager.getInstance().getVariable(ids.currency).getValue())
    );
    ...
    if (this.canLevelUp()) {
      ...
      if (this.getLevel() === 1) {
        GameManager.getInstance().getNotificationManager().addNotification({
          ...
          background: "https://i.imgur.com/uenbC1V.jpg",
          description:
            "Bow before me TINY HUMAN. I, the mightiest of dragons, require your strongest pets for a Mission!",
  ...
  getEventTimePassed(eventId: string): number {
    if (eventId === events.dogsploration.id) {
      return GameManager.getInstance()
        .getVariable(ids.product4Tier1Progress)
        .getValue();
    } else if (eventId === events.bigBoysploration.id) {
      return GameManager.getInstance()
        .getVariable(ids.product4Tier2Progress)
        .getValue();
    }
    return GameManager.getInstance()
      .getVariable(ids.product4Tier0Progress)
      .getValue();
  }
        ...
        relics: [
          {
            id: ids.relicTier0A,
            base: 5,
            icon: "https://i.imgur.com/DDm6ODQ.png",
          },
          {
            id: ids.relicTier0B,
            base: 1,
            icon: "https://i.imgur.com/0ZjGB7A.png",
          },
          {
            id: ids.relicTier0C,
            base: 1.5,
            icon: "https://i.imgur.com/qsbjSn1.png",
          },
          {
            id: ids.relicTier0D,
            base: 1.25,
      ...
      tier1: {
        ...
        relics: [
          // Increases Pet Power by 50%
          {
            id: ids.relicTier1A,
            base: 1.5,
            icon: "https://i.imgur.com/hfmuVYh.png",
          },
          // Lowers farm goal by 10%
          {
            id: ids.relicTier1B,
            base: 0.9,
            icon: "https://i.imgur.com/et49ttP.png",
          },
          // Doubles treats gained from alb
          {
            id: ids.relicTier1C,
            base: 2,
            icon: "https://i.imgur.com/d3CJRvY.png",
          },
          ...
          // Unlocks Museum
          {
            id: ids.relicMuseum,
            base: 1,
            icon: getBuildingIcon(ids.museum).background,
          },
        ],
      },
      tier2: {
        ...
        relics: [
          {
            id: ids.relicTier2SpecialBuildingA,
            base: 1,
            icon: "https://i.imgur.com/zYb3z9n.jpg",
          },
          //Increases Pet Power by 75%
          {
            id: ids.relicTier2A,
            base: 1.75,
            icon: "https://i.imgur.com/lYnJrc9.png",
          },
          //Doubles Treat Donations
          {
            id: ids.relicTier2B,
            base: 2,
            icon: "https://i.imgur.com/PaEoZHp.png",
          },
          //Doubles treats gained from lab
          {
            id: ids.relicTier2C,
            base: 2,
            icon: "https://i.imgur.com/UDLIwWS.png",
          },
  ...
  _getAllUnlockedRelicsFromTier(
    ...
    if (isUnlocked) {
      return tier.relics.filter((relic) => {
        const relicLvl = GameManager.getInstance()
          .getVariable(relic.id)
          .getValue();
        return relicLvl > 0;
      });
    }
