...
class GameManager {
  ...
  initializeUI() {
    ...
    const tree = this.getProductManager().getProduct(
      ids.treeOfGoodBoys
    ) as Tree;
    const bonus: number = tree.getBlessing(ids.blessing0B).getBonus();
    currencyOffline.patiencePoints = currencyOffline.patiencePoints
      ?.min(72)
      .mul(bonus);
    this.addCurrency(currencyOffline);
    if (this.getVariable(ids.lastSaveDate).getValue() != null) {
      const timesReset = Number(
        this.getPermaVariable(permaIds.timesReset)?.getValue()
      );
      const wizpugLevel = Number(
        this.getVariable(ids.product3Level)?.getValue()
      );
      if (timesReset > 0 && wizpugLevel <= 0) {
        const selectedDog = this.getPermaVariable(
          permaIds.selectedDogBreed
        )?.getValue();
        this.getNotificationManager().addNotification({
          id: "welcomed-back",
          background: getBuildingIcon(ids.product3Level).background,
          description:
            "IT IS!1!! They told me there was a human who gave puptastic PETS! HELLO HELLO!",
        ...
        this.getNotificationManager().addNotification({
          ...
          background: getBuildingIcon(ids.product3Level).background,
          description:
            "Your dogs have been waiting for you. They gained " +
            toFormat(
              currencyOffline.patiencePoints
                ? currencyOffline.patiencePoints
                : 0
            ) +
            " patience points for waiting like good boys. Spend them to speed up the game!",
      ...
      this.getNotificationManager().addNotification({
        ...
        description:
          "I've been waiting a long time for a good hooman to come. I heard they give great pets!",
  ...
  addCurrency(add: Currency): void {
    ...
    let myCurrency: Decimal = new Decimal(
      Number(this.getVariable(ids.currency).getValue())
    );
    let myTreats: Decimal = new Decimal(
      Number(this.getVariable(ids.treats).getValue())
    );
    let myPatiencePoints: Decimal = new Decimal(
      Number(this.getVariable(ids.patiencePoints).getValue())
    );
    myCurrency = myCurrency.add(currency);
    myTreats = myTreats.add(treats);

    if (patiencePoints) {
      myPatiencePoints = myPatiencePoints.add(
        patiencePoints ? patiencePoints : 0
      );
  ...
  getCurrency(): Currency {
    let currency: Decimal = new Decimal(
      Number(this.getVariable(ids.currency).getValue())
    );
    let treats: Decimal = new Decimal(
      Number(this.getVariable(ids.treats).getValue())
    );
    let patiencePoints: Decimal = new Decimal(
      Number(this.getVariable(ids.patiencePoints).getValue())
    );
    return {
      currency,
  ...
  onClickedDog(): Currency {
    const PetPetting = this.productManager.getProduct(
      ids.product0Level
    ) as PetPetting;
    ...
    const baseClickCurrency = currency
      .mul(devMegaPetMult)
      .mul(relic0ABonus !== 0 ? relic0ABonus : 1);
  ...
  handleOfflineTimePassed(): Currency {
    ...
    const diffInSeconds =
      Math.abs(today.getTime() - lastSaveTime.getTime()) / 1000;
    const patiencePointsGained = this.getTimeManager().getPatiencePoints(
      diffInSeconds
    );
  ...
  buyTurboTime(patienceSpent: Decimal) {
    const patiencePoints = new Decimal(
      Number(this.getVariable(ids.patiencePoints).getValue())
    );
  ...
  unlockTimeWizard() {
    ...
    if (dogWizardLvl < 1) {
      ...
      this.getNotificationManager().addNotification({
        ...
        background: getBuildingIcon(ids.product3Level).background,
        description:
          "Greetings human, it is I, the Wizpug! With my powers you can use your dogs stored patience to speed time ITSELF.",
  ...
  letGo() {
    ...
    const goodBoyPointsEarned = (this.productManager.getProduct(
      ids.treeOfGoodBoys
    ) as Tree).getGoodBoyPointsThisGame();
    const previosGbp = new Decimal(
      Number(this.getPermaVariable(permaVariables.goodBoypoints).getValue())
    );
    const newgpb = previosGbp.add(goodBoyPointsEarned);
    const timesReset =
      1 + Number(this.getPermaVariable(permaIds.timesReset).getValue());
    resetGameSave();
    this.variables = initializeVariables();
    this.permaVariables = initializePermaVariables();
...
const initializeVariables = () => {
  ...
  variablesObject[ids.currency] = new Variable({
    id: ids.currency,
    value: new Decimal(0),
  });
  variablesObject[ids.treats] = new Variable({
    id: ids.treats,
    value: new Decimal(0),
  });
...
const initializePermaVariables = () => {
  let variablesObject: VariableStructure = {};
  Object.keys(permaIds).forEach((id: string) => {
    variablesObject[id] = new Variable({ id, value: null });
  });
...
const loadSavedData = (
  variablesObject: VariableStructure
): VariableStructure => {
  ...
  if (save) {
    Object.keys(save).forEach((id: string) => {
...
const loadPermaSaveData = (
  permaVariablesObject: VariableStructure
): VariableStructure => {
...
const saveGame = (
  ...
  variables.lastSaveDate = new Variable({
    id: ids.lastSaveDate,
    value: new Date(),
  });
  const cook = Cookies.set(saveVarName, JSON.stringify(variables), {
    expires: 10 * 365,
  });
